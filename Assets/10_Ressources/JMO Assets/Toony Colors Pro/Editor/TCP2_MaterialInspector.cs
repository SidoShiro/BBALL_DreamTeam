...
public class TCP2_MaterialInspector : MaterialEditor
{
	//Constants
...
//				ShowFilteredProperties("#DAMB#", props);
		...
		if(updateVariant && !isGeneratedShader)
		{
			...
			if(shader != null)
			{
				SerializedProperty serializedKeywords = serializedObject.FindProperty("m_Shader");
	...
	private bool ShaderMaterialPropertyImpl(MaterialProperty property, string filter = null)
	{
		...
		case MaterialProperty.PropType.Texture:
			...
			if(!isGeneratedShader)
				showOffset = !nameLower.Contains("cubemap");

			TextureProperty(property, displayName, showOffset);
			break;
